/*********************************
  ...
  Node Helper for MMM-OpenWeatherForecastDeluxe.
    ...
    socketNotificationReceived: function(notification, payload) {
        if (notification === "OPENWEATHER_ONE_CALL_FORECAST_GET") {
            ...
            if (payload.apikey == null || payload.apikey == "") {
                console.log("[MMM-OpenWeatherForecastDeluxe] " + moment().format("D-MMM-YY HH:mm") + " ** ERROR ** No API key configured. Get an API key at https://openweathermap.org/api/one-call-api");
            } else if (payload.latitude == null || payload.latitude == "" || payload.longitude == null || payload.longitude == "") {
                console.log("[MMM-OpenWeatherForecastDeluxe] " + moment().format("D-MMM-YY HH:mm") + " ** ERROR ** Latitude and/or longitude not provided.");
            } else {
                ...
                // console.log("[MMM-OpenWeatherForecastDeluxe] Getting data: " + url);
                needle.get(url, function(error, response, body) {
                    ...
                    if (!error && response.statusCode == 200) {
                        ...
                        console.log("[MMM-OpenWeatherForecastDeluxe] " + moment().format("D-MMM-YY HH:mm") + " ** ERROR ** " + error);
                    }
